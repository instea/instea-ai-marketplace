...
|---|---|---|
| [project-setup](plugins/project-setup) | `docker-release-workflow` ⌨️<br>`devcontainer-claude-sandbox` ⌨️ | Sets up a GitHub Actions workflow that builds each service's Docker image, pushes it to `registry.instea.co`, and cuts a GitHub release from the `package.json` version — scaffolding any missing Dockerfile. Also scaffolds a `.devcontainer/` that runs Claude Code sandboxed inside the project's own container, keeping the agent's config on a named volume instead of the host's `~/.claude` — and declares this marketplace in the repo's `.claude/settings.json` so the house skills reach every container. |
